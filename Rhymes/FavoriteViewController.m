...
@interface FavoriteViewController () <UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate> {
    int nTableWidth;
    int nTableHeight;
    int nOffsets[100];
    BOOL bStart;
...
@property (weak, nonatomic) IBOutlet UIImageView *arrowStarView;
...
- (void)loadWordlist {
    ...
    NSArray *sorted = [_arrayWordpack sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        
        if ([obj1 isKindOfClass:[Wordlist class]] && [obj2 isKindOfClass:[Wordlist class]]) {
            Wordlist *s1 = obj1;
            Wordlist *s2 = obj2;
            
            if (s1.strFavTime.longLongValue > s2.strFavTime.longLongValue) {
    ...
    _arrayWordpack = [NSMutableArray arrayWithArray:sorted];
    ...
    for (int i = 0; i < _arrayWordpack.count; i ++) {
        ...
        if (wordlist.nid == [AppDelegate sharedAppDelegate].nFavPos)
            nPagePos = i;
    }
...
- (void)loadTableView:(int)nPage {
    ...
    [AppDelegate sharedAppDelegate].nFavPos = wordlist.nid;
    if (wordlist.bFavorite) {
        [_favoriteBut setImage:[UIImage imageNamed:@"starSelected"] forState:UIControlStateNormal];
    }
    else {
